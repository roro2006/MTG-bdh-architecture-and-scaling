...
6. **Zero-shot probe** at one size: train on $n-1$ sets, test on the held-out one. *Run for the cross-attention arm; see `RESULTS.md`.* Trained on eight sets (`AFR` cannot be one — see `DATA.md`) and evaluated on `FIN`'s val split at $d=64$: **1.6916 loss / 40.2% on picks 0–8**, against a same-set ceiling of 1.0033 / 62.4% and a transferred pick-rate prior of 2.2302 / 9.5%. It clears the same-set prior (1.9474 / 36.3%) that is fitted on the held-out set's own drafts. **The BDH arm is outstanding**, so §8's deliverable — zero-shot numbers "against both reference points" — is met, but the arm comparison this step could also have supported is not. Two findings worth carrying forward: held-out loss peaks at 57% of the step budget where the same-set run peaked at 96%, and the transfer gap is widest on the earliest, hardest picks.
