...
6. ~~**Zero-shot probe** at one size: train on $n-1$ sets, test on the held-out one.~~ Done, both arms; see `RESULTS.md`. Trained on eight sets (`AFR` cannot be one — see `DATA.md`) and evaluated on `FIN`'s val split at $d=64$: **1.6478 / 41.6% (BDH) and 1.6916 / 40.2% (attention) on picks 0–8**, against a same-set ceiling of 1.0037 / 62.5% and a transferred pick-rate prior of 2.2302 / 9.5%. Both clear the same-set prior (1.9474 / 36.3%) that is fitted on the held-out set's own drafts, which is the strongest single statement available: a drafter that has never seen the set beats a card-quality table computed from 4.7M of its picks. Three findings to carry forward. **The arms tie in-distribution (0.0004), and held out they do not separate either**: BDH led by 0.0438 at seed 0, but the seed replication moved cross-attention alone by 0.0595 at matched steps — about twice the arm gap — so that lead was seed noise and is retracted in `RESULTS.md`. **§6's grid therefore needs more than one seed per cell**, which is a budget change, not a detail: held-out variance ran over a hundred times the in-distribution variance. Held-out loss peaks near half the step budget where the same-set run peaked at 96%. And the transfer gap is widest on the earliest, hardest picks.
